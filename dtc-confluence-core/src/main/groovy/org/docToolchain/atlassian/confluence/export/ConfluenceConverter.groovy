 ...
 *
 * <p>Moved unchanged in behaviour from docToolchain's scripts/confluenceConverter.groovy, where it
 * was a set of closures shared between two drivers through a Gradle binding. The closures are
 * methods here and the binding variables are fields, so the conversion can be called - and tested -
 * without a build tool around it.</p>
 *
 * <p>Both export drivers use it: the XML space export and the REST API partial export.</p>
 */
class ConfluenceConverter {

    /** Counts the storage-format tags the conversion did not recognise, for the driver to report. */
    Map unknownTagsStats = [:]

    /** Whether a LucidChart reference becomes an iframe rather than an image. */
    boolean lucidChartsIframe = false

    /** Where discovered LucidChart references are logged; null to log none. */
    File lucidInfoFile = null

    /**
     * Strips a hand-written chapter number from a heading ("5.2.4. Title" -> "Title").
     * AsciiDoc can number sections itself, so the prefix carried over from Confluence is noise.
     */
    boolean stripChapterNumbering = true
    ...
    def findRootIdByTitle(Map pages, String title) {
    ...
    def filterToSubtree(Map pages, Map attachments, String rootPageId) {
    ...
    def fixBody(String pageId, String body, Map users, Map pages, Map attachments, Map space) {
        body = body
...
${lucidInfos.replaceAll("\n", "%%CRLF%%")}
    ...
    def writePage(String pageId, String rawBody, List<String> childIds,
...
"""
    ...
    def createMenu(Map pages, startPageId) {
